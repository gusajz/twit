...
var Twitter = function (config) {
  ...
  this.useCompression = config.gzip || false;

  //authenticate our oauth client
  // this.auth = new Auth(credentials, this.useCompression);
...
Twitter.prototype = {
  ...
  request: function (method, path, params, callback) {
    ...
    return new OARequest(new Auth(this.credentials, this.useCompression), method, finalPath + '.json', finalParams).end(callback)
  },
  stream: function (path, params) {
    var ROOT = STREAM_ENDPOINT_MAP[path] || PUB_STREAM
    var streamPath = ROOT + path + '.json'

    return new OARequest(new Auth(this.credentials, this.useCompression), 'POST', streamPath, this.normalizeParams(params)).persist()
  },
