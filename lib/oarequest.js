...
OARequest.prototype.keepAlive = function () {
  ...
  this.request.once('response', function (response) {
    ...
    if (self.response.headers['content-encoding'] === 'gzip') {
      consumerResponse = self.response.pipe(zlib.createGunzip());
    } else {
      consumerResponse = self.response;
    }

    self.emit('connected', self.response);
    consumerResponse.setEncoding('utf8');


    //pass all response data to parser
    consumerResponse.on('data', function (chunk) {
    ...
    consumerResponse.once('close', function () {
      self.scheduleStreamReconnect(self.request, self.response);
    });

    consumerResponse.on('error', function (err) {
...
OARequest.prototype.end = function (callback) {
  ...
  this.request.once('response', function(response) {
    ...
    var writeAll = concat(function(data) {
      // parse response
      var parsed = null
        , parseError = null
      ...
      if (parseError) {
        ...
        badReplyError.twitterReply = data
      ...
      if (response.statusCode !== 200) {
        ...
        try {
          var parsedError = JSON.parse(data);
  ...
  this.request.once('error', function (err, data) {
